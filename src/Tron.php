...
use Jackillll\Tron\Exception\TRC20Exception;
use Jackillll\Tron\Support\Base58;
use Jackillll\Tron\Support\Base58Check;
use Jackillll\Tron\Support\Crypto;
use Jackillll\Tron\Support\Hash;
use Jackillll\Tron\Support\Keccak;
use Jackillll\Tron\Support\Utils;
use Jackillll\Tron\Provider\HttpProviderInterface;
use Jackillll\Tron\Exception\TronException;

/**
 * Main Tron API class for interacting with the Tron (TRX) blockchain
 *
 * This class provides comprehensive functionality for:
 * - Account management and address validation
 * - Transaction creation and broadcasting
 * - Smart contract interaction (TRC20, TRC721, etc.)
...
class Tron implements TronInterface
{
    ...
    */
     ...
     * 
     * @param array $votes Array of votes in format [['vote_address' => 'address', 'vote_count' => count], ...]
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function voteWitnessAccount(array $votes, ?string $ownerAddress = null): array
    {
        if (empty($votes)) {
            throw new TronException('Votes array cannot be empty');
        }

        $owner = $ownerAddress ?: $this->address['hex'];
        if (!$owner) {
        ...
        foreach ($votes as $vote) {
            if (!isset($vote['vote_address']) || !isset($vote['vote_count'])) {
                throw new TronException('Each vote must contain vote_address and vote_count');
            }
            
            if (!is_numeric($vote['vote_count']) || $vote['vote_count'] <= 0) {
                throw new TronException('Vote count must be a positive number');
            }
        }

        return $this->manager->request('/wallet/votewitnessaccount', [
            'owner_address' => $owner,
            'votes' => $votes
        ]);
    }
     ...
     * 
     * @param string|null $address Account address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function getAccountVotes(?string $address = null): array
    {
        $accountAddress = $address ?: $this->address['hex'];
        if (!$accountAddress) {
            throw new TronException('Account address is required');
        }

        $account = $this->getAccount($accountAddress);
    ...
    {
        return $this->manager->request('/wallet/listwitnesses');
    }

    /**
     * Get witness information by address
     * 
     * @param string $address Witness address
     * @return array
     * @throws TronException
     */
    public function getWitness(string $address): array
    {
        if (empty($address)) {
            throw new TronException('Witness address is required');
        }

        return $this->manager->request('/wallet/getwitness', [
            'address' => $address
        ]);
    }

    /**
     * Get brokerage information for a witness
     * 
     * @param string $address Witness address
     * @return array
     * @throws TronException
     */
    public function getBrokerage(string $address): array
    {
        if (empty($address)) {
            throw new TronException('Witness address is required');
        }

        return $this->manager->request('/wallet/getBrokerage', [
            'address' => $address
        ]);
    }
     ...
     * @throws TronException
     */
    public function getReward(?string $address = null): array
    {
        $accountAddress = $address ?: $this->address['hex'];
        if (!$accountAddress) {
            throw new TronException('Account address is required');
        ...
        return $this->manager->request('/wallet/getReward', [
            'address' => $accountAddress
        ]);
    }

    /**
     * Freeze balance V2 (Stake 2.0) - Stake TRX to obtain resources
     * 
     * @param int $frozenBalance Amount to freeze in SUN (1 TRX = 1,000,000 SUN)
     * @param string $resource Resource type: 'BANDWIDTH' or 'ENERGY'
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function freezeBalanceV2(int $frozenBalance, string $resource = 'BANDWIDTH', ?string $ownerAddress = null): array
    {
        if ($frozenBalance <= 0) {
            throw new TronException('Frozen balance must be greater than 0');
        }

        if (!in_array($resource, ['BANDWIDTH', 'ENERGY'])) {
            throw new TronException('Resource must be either BANDWIDTH or ENERGY');
        }
        ...
        return $this->manager->request('/wallet/freezebalancev2', [
            'owner_address' => $owner,
    ...
    /**
     * Unfreeze balance V2 (Stake 2.0) - Unstake TRX
     * 
     * @param int $unfreezeBalance Amount to unfreeze in SUN
     * @param string $resource Resource type: 'BANDWIDTH' or 'ENERGY'
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function unfreezeBalanceV2(int $unfreezeBalance, string $resource = 'BANDWIDTH', ?string $ownerAddress = null): array
    {
        if ($unfreezeBalance <= 0) {
            throw new TronException('Unfreeze balance must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/unfreezebalancev2', [
            'owner_address' => $owner,
     ...
     * 
     * @param string $receiverAddress Address to delegate resources to
     * @param int $balance Amount to delegate in SUN
     * @param string $resource Resource type: 'BANDWIDTH' or 'ENERGY'
    ...
    public function delegateResource(string $receiverAddress, int $balance, string $resource = 'BANDWIDTH', bool $lock = false, int $lockPeriod = 0, ?string $ownerAddress = null, ?int $permission_id = null): array
    {
        ...
        if ($balance <= 0) {
            throw new TronException('Balance must be greater than 0');
        }
        ...
        if ($lock && $lockPeriod <= 0) {
            throw new TronException('Lock period must be greater than 0 when lock is enabled');
        }
    ...
    /**
     * Undelegate resource from another account
     * 
     * @param string $receiverAddress Address to undelegate resources from
     ...
     * 
     * @param string $fromAddress Address that delegated the resources
     * @param string $toAddress Address that received the resources
     * @return array
     * @throws TronException
     */
    public function getDelegatedResource(string $fromAddress, string $toAddress): array
    {
        if (empty($fromAddress) || empty($toAddress)) {
            throw new TronException('Both from and to addresses are required');
        }

        return $this->manager->request('/wallet/getdelegatedresource', [
    ...
    公共 function getDelegatedResourceAccountIndex(?string $address = null): array
    {
        ...
        return $this->manager->request('/wallet/getdelegatedresourceaccountindex', [
            'address' => $accountAddress
        ]);
    }

    /**
     * Get available unfreeze count for Stake 2.0
     * 
     * @param string|null $address Account address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function getAvailableUnfreezeCount(?string $address = null): array
    {
        $accountAddress = $address ?: $this->address['hex'];
        if (!$accountAddress) {
            throw new TronException('Account address is required');
    ...
    /**
     * Cancel all unfreezes for Stake 2.0
     * 
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function cancelAllUnfreezeV2(?string $ownerAddress = null): array
    {
        $owner = $ownerAddress ?: $this->address['hex'];
        if (!$owner) {
            throw new TronException('Owner address is required');
        ...
        return $this->manager->request('/wallet/cancelallunfreezev2', [
            'owner_address' => $owner
        ]);
    }

    /**
     * Create a proposal for network governance
     * 
     * @param array $parameters Array of parameters to propose changes for
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function createProposal(array $parameters, ?string $ownerAddress = null): array
    {
        if (empty($parameters)) {
            throw new TronException('Parameters array cannot be empty');
        }
        ...
        return $this->manager->request('/wallet/proposalcreate', [
            'owner_address' => $owner,
     ...
     * Approve a proposal
     * 
     * @param int $proposalId Proposal ID to approve
     * @param bool $isApproval Whether to approve (true) or disapprove (false)
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function approveProposal(int $proposalId, bool $isApproval = true, ?string $ownerAddress = null): array
    {
        if ($proposalId <= 0) {
            throw new TronException('Proposal ID must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/proposalapprove', [
            'owner_address' => $owner,
     ...
     * Delete a proposal
     * 
     * @param int $proposalId Proposal ID to delete
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function deleteProposal(int $proposalId, ?string $ownerAddress = null): array
    {
        if ($proposalId <= 0) {
            throw new TronException('Proposal ID must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/proposaldelete', [
            'owner_address' => $owner,
            'proposal_id' => $proposalId
        ]);
    }
    ...
    {
        return $this->manager->request('/wallet/listproposals');
    }
    ...
    public function getProposal(int $proposalId): array
    {
        ...
        return $this->manager->request('/wallet/getproposal', [
            'id' => $proposalId
        ]);
    }
    ...
    {
        return $this->manager->request('/wallet/getchainparameters');
    }
    ...
    {
        return $this->manager->request('/wallet/getnodeinfo');
    }
     ...
     * @return array
     */
    public function getBurnTrx(): array
    {
        return $this->manager->request('/wallet/getburntrx');
    }
    ...
    {
        return $this->manager->request('/wallet/getenergyprices');
    }
    ...
    {
        return $this->manager->request('/wallet/getbandwidthprices');
    }

    /**
     * Create an exchange
     * 
     * @param string $firstTokenId First token ID
     * @param int $firstTokenBalance First token balance
     * @param string $secondTokenId Second token ID
     * @param int $secondTokenBalance Second token balance
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function exchangeCreate(string $firstTokenId, int $firstTokenBalance, string $secondTokenId, int $secondTokenBalance, ?string $ownerAddress = null): array
    {
        if (empty($firstTokenId) || empty($secondTokenId)) {
            throw new TronException('Both token IDs are required');
        }

        if ($firstTokenBalance <= 0 || $secondTokenBalance <= 0) {
            throw new TronException('Token balances must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/exchangecreate', [
            'owner_address' => $owner,
     ...
     * @param string $tokenId Token ID to inject
     * @param int $quant Amount to inject
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function exchangeInject(int $exchangeId, string $tokenId, int $quant, ?string $ownerAddress = null): array
    {
        if ($exchangeId <= 0) {
            throw new TronException('Exchange ID must be greater than 0');
        }

        if (empty($tokenId)) {
            throw new TronException('Token ID is required');
        }

        if ($quant <= 0) {
            throw new TronException('Quantity must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/exchangeinject', [
            'owner_address' => $owner,
    ...
    /**
     * Withdraw tokens from an exchange
     * 
     * @param int $exchangeId Exchange ID
     * @param string $tokenId Token ID to withdraw
     * @param int $quant Amount to withdraw
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function exchangeWithdraw(int $exchangeId, string $tokenId, int $quant, ?string $ownerAddress = null): array
    {
        if ($exchangeId <= 0) {
            throw new TronException('Exchange ID must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/exchangewithdraw', [
            'owner_address' => $owner,
            'exchange_id' => $exchangeId,
            'token_id' => $tokenId,
            'quant' => $quant
    ...
    /**
     * Execute an exchange transaction
     * 
     * @param int $exchangeId Exchange ID
     * @param string $tokenId Token ID to exchange
     * @param int $quant Amount to exchange
     * @param int $expected Expected amount to receive
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function exchangeTransaction(int $exchangeId, string $tokenId, int $quant, int $expected, ?string $ownerAddress = null): array
    {
        if ($exchangeId <= 0) {
            throw new TronException('Exchange ID must be greater than 0');
        }
        ...
        if ($quant <= 0 || $expected <= 0) {
            throw new TronException('Quantity and expected amount must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/exchangetransaction', [
            'owner_address' => $owner,
            'exchange_id' => $exchangeId,
            'token_id' => $tokenId,
            'quant' => $quant,
    ...
    public function getExchange(int $exchangeId): array
    {
        ...
        return $this->manager->request('/wallet/getexchange', [
            'id' => $exchangeId
        ]);
    }

    /**
     * Sell asset on the market
     * 
     * @param string $sellTokenId Token ID to sell
     * @param int $sellTokenQuantity Quantity to sell
     * @param string $buyTokenId Token ID to buy
     * @param int $buyTokenQuantity Quantity to buy
     * @param string|null $ownerAddress Owner address (optional, uses default if not provided)
     * @return array
     * @throws TronException
     */
    public function marketSellAsset(string $sellTokenId, int $sellTokenQuantity, string $buyTokenId, int $buyTokenQuantity, ?string $ownerAddress = null): array
    {
        if (empty($sellTokenId) || empty($buyTokenId)) {
            throw new TronException('Both sell and buy token IDs are required');
        }

        if ($sellTokenQuantity <= 0 || $buyTokenQuantity <= 0) {
            throw new TronException('Token quantities must be greater than 0');
        }
        ...
        return $this->manager->request('/wallet/marketsellasset', [
            'owner_address' => $owner,
     ...
     * @throws TronException
     */
    public function marketCancelOrder(string $orderId, ?string $ownerAddress = null): array
    {
        if (empty($orderId)) {
            throw new TronException('Order ID is required');
        }
        ...
        return $this->manager->request('/wallet/marketcancelorder', [
            'owner_address' => $owner,
            'order_id' => $orderId
        ]);
    }
     ...
     * @throws TronException
     */
    public function getMarketOrderById(string $orderId): array
    {
        if (empty($orderId)) {
            throw new TronException('Order ID is required');
        }

        return $this->manager->request('/wallet/getmarketorderbyid', [
            'order_id' => $orderId
        ]);
    }
     ...
     * @throws TronException
     */
    public function getMarketOrderByAccount(?string $address = null): array
    {
        $accountAddress = $address ?: $this->address['hex'];
        if (!$accountAddress) {
            throw new TronException('Account address is required');
        ...
        return $this->manager->request('/wallet/getmarketorderbyaccount', [
            'address' => $accountAddress
        ]);
    }
     ...
     * 
     * @param string $sellTokenId Sell token ID
     * @param string $buyTokenId Buy token ID
     * @return array
     * @throws TronException
     */
    public function getMarketPriceByPair(string $sellTokenId, string $buyTokenId): array
    {
        if (empty($sellTokenId) || empty($buyTokenId)) {
            throw new TronException('Both sell and buy token IDs are required');
        }

        return $this->manager->request('/wallet/getmarketpricebypair', [
            'sell_token_id' => $sellTokenId,
            'buy_token_id' => $buyTokenId
        ]);
    }
     ...
     * @throws TronException
     */
    public function getMarketOrderListByPair(string $sellTokenId, string $buyTokenId): array
    {
        if (empty($sellTokenId) || empty($buyTokenId)) {
            throw new TronException('Both sell and buy token IDs are required');
        }

        return $this->manager->request('/wallet/getmarketorderlistbypair', [
            'sell_token_id' => $sellTokenId,
            'buy_token_id' => $buyTokenId
        ]);
    }
     ...
     * 
     * @param array $shieldedSpends Array of shielded spend parameters
     * @param array $shieldedReceives Array of shielded receive parameters
     * @param string|null $transparentFromAddress Transparent from address (optional)
     * @param int|null $transparentToAddress Transparent to address (optional)
     * @param int|null $fromAmount Amount from transparent address (optional)
     * @param int|null $toAmount Amount to transparent address (optional)
     * @return array
     * @throws TronException
     */
    public function createShieldedTransaction(array $shieldedSpends = [], array $shieldedReceives = [], ?string $transparentFromAddress = null, ?int $transparentToAddress = null, ?int $fromAmount = null, ?int $toAmount = null): array
    {
        $params = [];

        if (!empty($shieldedSpends)) {
            $params['shielded_spends'] = $shieldedSpends;
        }

        if (!empty($shieldedReceives)) {
            $params['shielded_receives'] = $shieldedReceives;
        }

        if ($transparentFromAddress) {
            $params['transparent_from_address'] = $transparentFromAddress;
        }

        if ($transparentToAddress) {
            $params['transparent_to_address'] = $transparentToAddress;
        }

        if ($fromAmount !== null) {
            $params['from_amount'] = $fromAmount;
        }

        if ($toAmount !== null) {
            $params['to_amount'] = $toAmount;
        }

        if (empty($params)) {
            throw new TronException('At least one parameter is required for shielded transaction');
        }

        return $this->manager->request('/wallet/createshieldedtransaction', $params);
    }

    /**
     * Get shielded transaction hash
     * 
     * @param array $transaction Shielded transaction data
     * @return array
     * @throws TronException
     */
    public function getShieldedTransactionHash(array $transaction): array
    {
        if (empty($transaction)) {
            throw new TronException('Transaction data is required');
        }

        return $this->manager->request('/wallet/getshieldedtransactionhash', $transaction);
    }
     ...
     * @throws TronException
     */
    public function createShieldedContractParameters(array $shieldedSpends = [], array $shieldedReceives = [], ?string $transparentFromAddress = null, ?int $transparentToAddress = null, ?int $fromAmount = null, ?int $toAmount = null): array
    {
        $params = [];
        ...
        if (empty($params)) {
            throw new TronException('At least one parameter is required for shielded contract parameters');
        }

        return $this->manager->request('/wallet/createshieldedcontractparameters', $params);
    }
     ...
     * @throws TronException
     */
    public function createShieldedContractParametersWithoutAsk(array $shieldedSpends = [], array $shieldedReceives = [], ?string $transparentFromAddress = null, ?int $transparentToAddress = null, ?int $fromAmount = null, ?int $toAmount = null): array
    {
        $params = [];
        ...
        return $this->manager->request('/wallet/createshieldedcontractparameterswithoutask', $params);
    }

    /**
     * Scan note by IVK (Incoming Viewing Key)
     * 
     * @param string $ivk Incoming viewing key
     * @param string $ak Authorizing key
     * @param string $nk Nullifier key
     * @param int $startBlockNumber Start block number
     * @param int $endBlockNumber End block number
     * @return array
     * @throws TronException
     */
    public function scanNoteByIvk(string $ivk, string $ak, string $nk, int $startBlockNumber, int $endBlockNumber): array
    {
        if (empty($ivk) || empty($ak) || empty($nk)) {
            throw new TronException('IVK, AK, and NK are required');
        }

        if ($startBlockNumber < 0 || $endBlockNumber < 0 || $startBlockNumber > $endBlockNumber) {
            throw new TronException('Invalid block number range');
        }

        return $this->manager->request('/wallet/scannotebyivk', [
            'ivk' => $ivk,
            'ak' => $ak,
            'nk' => $nk,
     ...
     * @throws TronException
     */
    public function scanAndMarkNoteByIvk(string $ivk, string $ak, string $nk, int $startBlockNumber, int $endBlockNumber): array
    {
        if (empty($ivk) || empty($ak) || empty($nk)) {
            throw new TronException('IVK, AK, and NK are required');
        }
        ...
        return $this->manager->request('/wallet/scanandmarknotebyivk', [
            'ivk' => $ivk,
            'ak' => $ak,
            'nk' => $nk,
            'start_block_number' => $startBlockNumber,
    ...
    /**
     * Scan note by OVK (Outgoing Viewing Key)
     * 
     * @param string $ovk Outgoing viewing key
     * @param int $startBlockNumber Start block number
     * @param int $endBlockNumber End block number
     * @return array
     * @throws TronException
     */
    public function scanNoteByOvk(string $ovk, int $startBlockNumber, int $endBlockNumber): array
    {
        if (empty($ovk)) {
            throw new TronException('OVK is required');
        }
        ...
        return $this->manager->request('/wallet/scannotebyovk', [
            'ovk' => $ovk,
            'start_block_number' => $startBlockNumber,
            'end_block_number' => $endBlockNumber
        ]);
     ...
     * @return array
     */
    public function getSpendingKey(): array
    {
        return $this->manager->request('/wallet/getspendingkey');
    }
     ...
     * @throws TronException
     */
    public function getExpandedSpendingKey(string $spendingKey): array
    {
        if (empty($spendingKey)) {
            throw new TronException('Spending key is required');
        }

        return $this->manager->request('/wallet/getexpandedspendingkey', [
            'spending_key' => $spendingKey
        ]);
    }

    /**
     * Get AK from ASK (Authorizing Key from Authorizing Spending Key)
     * 
     * @param string $ask Authorizing spending key
     * @return array
     * @throws TronException
     */
    public function getAkFromAsk(string $ask): array
    {
        if (empty($ask)) {
            throw new TronException('ASK is required');
        }

        return $this->manager->request('/wallet/getakfromask', [
            'ask' => $ask
        ]);
    }

    /**
     * Get NK from NSK (Nullifier Key from Nullifier Spending Key)
     * 
     * @param string $nsk Nullifier spending key
     * @return array
     * @throws TronException
     */
    public function getNkFromNsk(string $nsk): array
    {
        if (empty($nsk)) {
            throw new TronException('NSK is required');
        }

        return $this->manager->request('/wallet/getnkfromnsk', [
            'nsk' => $nsk
        ]);
    }
     ...
     * @throws TronException
     */
    public function getIncomingViewingKey(string $ak, string $nk): array
    {
        if (empty($ak) || empty($nk)) {
            throw new TronException('AK and NK are required');
        }

        return $this->manager->request('/wallet/getincomingviewingkey', [
            'ak' => $ak,
            'nk' => $nk
        ]);
    }
     ...
     * @return array
     */
    public function getDiversifier(): array
    {
        return $this->manager->request('/wallet/getdiversifier');
    }
     ...
     * 
     * @param string $ivk Incoming viewing key
     * @param string $diversifier Diversifier
     * @return array
     * @throws TronException
     */
    public function getShieldedPaymentAddress(string $ivk, string $diversifier): array
    {
        if (empty($ivk) || empty($diversifier)) {
            throw new TronException('IVK and diversifier are required');
        }

        return $this->manager->request('/wallet/getshieldedpaymentaddress', [
            'ivk' => $ivk,
    ...
    /**
     * Get RCM (Randomness for Commitment)
     * 
     * @return array
     */
    public function getRcm(): array
    {
        return $this->manager->request('/wallet/getrcm');
    }
     ...
     * @param string $ak Authorizing key
     * @param string $nk Nullifier key
     * @param int $position Note position
     * @return array
     * @throws TronException
     */
    public function isNoteSpent(string $ak, string $nk, int $position): array
    {
        if (empty($ak) || empty($nk)) {
            throw new TronException('AK and NK are required');
        }
        ...
        if ($position < 0) {
            throw new TronException('Position must be non-negative');
        }

        return $this->manager->request('/wallet/isnotespent', [
            'ak' => $ak,
            'nk' => $nk,
            'position' => $position
        ]);
    ...
    /**
     * Create shielded nullifier
     * 
     * @param string $nsk Nullifier spending key
     * @param int $value Note value
     * @param int $position Note position
     * @param string $cm Commitment
     * @return array
     * @throws TronException
     */
    public function createShieldedNullifier(string $nsk, int $value, int $position, string $cm): array
    {
        if (empty($nsk) || empty($cm)) {
            throw new TronException('NSK and CM are required');
        }

        if ($value < 0 || $position < 0) {
            throw new TronException('Value and position must be non-negative');
        }

        return $this->manager->request('/wallet/createshieldednullifier', [
            'nsk' => $nsk,
            'value' => $value,
            'position' => $position,
            'cm' => $cm
